...
c      close(12)
c      do i=2,kkbeta
c      write(12,2010) r(i),qfunc(i,1,1),qfunc(i,2,2),
c     &  qfunc(i,1,2),qfunc(i,3,3),qfunc(i,1,3),qfunc(i,i,4)
c      enddo
      sum_tot=0.d0
      do i=2,mesh-1
      sum=0.d0
      do j=1,nvalps
      sum=sum+(snl(i,j)/r(i))**2*wwnlps(j)
      enddo
      sum_tot=sum_tot+rsatom(i)*(r(i+1)-r(i-1))/2
      write(12,2010) r(i),sum,rsatom(i)/r(i)**2
      enddo

      write(6,*) "sum_tot=", sum_tot
